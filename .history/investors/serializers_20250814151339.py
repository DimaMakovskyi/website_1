...
class SavedStartupSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        # Явна перевірка “не можна зберегти власний стартап”
        ...
        # Якщо у БД notes = NOT NULL – гарантуємо рядок (інакше можна лишити як є)
        ...
        try:
            with transaction.atomic():
                ...
                # Викликаємо модельну валідацію, яка включає SavedStartup.clean()
                try:
                    obj.full_clean()  # підхопить "You cannot save your own startup." з моделі
                except DjangoValidationError as e:
                    # Перетворюємо на DRF-помилку
                    raise serializers.ValidationError(e.message_dict or {'non_field_errors': e.messages})
                obj.save()
                return obj
        except IntegrityError:
            # Ловимо дубль (гонку) і повертаємо дружнє повідомлення
