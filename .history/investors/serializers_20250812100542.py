...
class SavedStartupSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = SavedStartup
        fields = ['id', 'investor_id', 'startup', 'startup_id', 'status', 'notes', 'saved_at', 'created_at', 'updated_at']
    ...
    def create(self, validated_data):
        investor = validated_data.pop('investor', None) \
                   or self.context['request'].user.investor
